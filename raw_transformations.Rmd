...
And finally, decide if a peak is noise or not based on the new cutoff, and compare
that to it's old status of noise or not, and get list of new peaks.

```{r check_noise_status}
check_noise_status <- function(normalized_peaks, threshold) {
  normalized_peaks$not_noise2 <- TRUE
  normalized_peaks[normalized_peaks$Height <= threshold, "not_noise2"] <- FALSE
  normalized_peaks
}
```
...
noise_rechecked <- purrr::map2(normed_peak_lists, max_noise, check_noise_status)

purrr::map_df(noise_rechecked, function(x){
  data.frame(old = sum(x$not_noise), new = sum(x$not_noise2))
})
```
